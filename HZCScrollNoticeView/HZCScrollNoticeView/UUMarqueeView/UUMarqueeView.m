...
- (void)scrollTimerDidFire:(NSTimer *)timer {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
        if (self.direction == UUMarqueeViewDirectionLeftward) {
            [self moveToNextDataIndex];
            ...
            __weak __typeof(self) weakSelf = self;
            [UIView animateWithDuration:(currentItemWidth / self.scrollSpeed) delay:1.0 options:UIViewAnimationOptionCurveLinear animations:^{
                CGFloat lastMaxX = 0.0f;
                for (int i = 0; i < self.items.count; i++) {
                    int index = (i + self.firstItemIndex) % self.items.count;
